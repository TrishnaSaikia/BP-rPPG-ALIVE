...
Official repository structure for:

**BP-rPPG: An Indian Face-Video Dataset and PPG-Guided Baseline for Remote Blood Pressure Estimation**

The repository provides the dataset-access instructions, paper-aligned teacher and student architectures, preprocessing utilities, teacher training, student training, and student-only testing with quality-weighted fusion.

**No pretrained teacher or student weights are distributed.** Users first train the PPG teacher and then use its generated checkpoint to train ALIVE.

## Method overview

ALIVE contains two networks:

- `N_PPG`: a PPG-based BP network trained using 4-second contact-PPG clips.
- `N_rPPG`: the facial-video student network trained using the synchronized PPG teacher for feature guidance.

During student training, the frozen teacher provides `F_p` and the student produces `F_r`. The student is optimized using:

```text
L_C = L_F + L_DF
```

where `L_F` is negative Pearson feature alignment and `L_DF` is the squared SBP/DBP data-fidelity loss. During testing, the teacher and contact PPG are not used. Clip-wise student predictions are combined using the paper's quality-weighted fusion.
